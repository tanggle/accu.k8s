...
resource "aws_instance" "k8s-lb" {
  ...
  #      "echo 'centos:AccuInsight*0' | sudo chpasswd"
  #    ]
  #  }

  #  provisioner "local-exec" {
  #    command = "sshpass -p'AccuInsight*0' ssh-copy-id -i ~/.ssh/id_rsa.pub -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null centos@${self.public_ip}"
  #  }
}
...
data "template_file" "inventory" {
  ...
  vars = {
    list_connstr_master = join("\n",
      formatlist(
        "%s ansible_host=%s ansible_user=centos private_ip=%s",
